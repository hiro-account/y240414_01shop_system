...
function get_content($prm_post) {
    ...
    try {
        ...
        $m_staff_result = $m_staff_stmt->execute($arr);
        ...
        // if ($staff_max_id !== $bf_staff_max_id + I_1) {
        //     return CREATE_FAILED;
        // }

        // 権限テーブルに登録後のidの最大値をidとして権限を登録する
        $t_privilege_stmt = $cmn_pdo->prepare('INSERT INTO ' . T_PRIVILEGE . ' (id, privilege, creator_id) VALUES (?, ?, ?)');
        $t_privilege_result = $t_privilege_stmt->execute(array($staff_max_id, $_POST['rdo_privilege'], $_SESSION[STAFF_ID]));
...
//         // スタッフの登録後のスタッフマスタのidの最大値を読み出し
//         $staff_max_id = read_max_id($mysqli, 'm_staff_for_dev');
...
//         // 権限テーブルに登録後のidの最大値をidとして権限を登録する
//         if ($t_privilege_stmt = $mysqli->prepare('INSERT INTO t_privilege_for_dev (id, privilege, creator_id) VALUES (?, ?, ?)')) {
// //             $t_privilege_stmt->bind_param('isi', $staff_max_id, $_SESSION['privilege'], $_SESSION[STAFF_ID]);
//             $t_privilege_stmt->bind_param('isi', $staff_max_id, $_POST['rdo_privilege'], $_SESSION[STAFF_ID]);
//             $t_privilege_stmt->execute();
//             $t_privilege_stmt->close();
//         }

//         // 権限の登録後の権限テーブルのidの最大値＝スタッフマスタのidの最大値であるはず
//         if (read_max_id($mysqli, 't_privilege_for_dev') !== $staff_max_id) {
//             // 登録後のidの最大値が正しくない場合
//             $mysqli->close();
...
//             $stmt_for_t_pswd->bind_param('isi', $staff_max_id, $pswd, $_SESSION[STAFF_ID]);
//             $stmt_for_t_pswd->execute();
//             $stmt_for_t_pswd->close();
//         }

//         // 仮パスワードの登録後のパスワードテーブルのidの最大値＝スタッフマスタのidの最大値であるはず
//         if (read_max_id($mysqli, 't_password_for_dev') !== $staff_max_id) {
//             // 登録後のidの最大値が正しくない場合
//             $mysqli->close();
...
//             $stmt_for_t_lgcl_del->bind_param('ii', $staff_max_id, $_SESSION[STAFF_ID]);
//             $stmt_for_t_lgcl_del->execute();
//             $stmt_for_t_lgcl_del->close();
//         }

//         // 登録後の削除ステータステーブルのidの最大値＝スタッフマスタのidの最大値であるはず
//         if (read_max_id($mysqli, 't_logical_delete_for_dev') !== $staff_max_id) {
//             // 登録後のidの最大値が正しくない場合
//             $mysqli->close();
